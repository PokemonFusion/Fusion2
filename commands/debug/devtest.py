...
def _get_unverified_moves() -> List[str]:
	...
	try:  # pragma: no cover - DB access not available in some tests
		...
		return [name for name in Move.objects.values_list("name", flat=True) if not status.get(name.lower(), False)]
	except Exception:
...
def _start_ephemeral_battle(caller, atk_pkmn, def_pkmn):
	...
	try:
		...
		battle._init_battle_state(caller.location, [atk], defender, opponent.key, BattleType.TRAINER)
...
class CmdTestBattle(Command):
	...
	def func(self):
		...
		try:
			testmon = make_test_pokemon(level=level, moves=moves, ability=ability, item=item, seed=seed)
