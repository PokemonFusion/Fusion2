...
class CmdExamine(DefaultCmdExamine):
	...
	def format_single_attribute_detail(self, obj, attr):
		...
		if not building._FUNCPARSER:
			building._FUNCPARSER = funcparser.FuncParser(settings.FUNCPARSER_OUTGOING_MESSAGES_MODULES)
		...
		return f"Attribute {obj.name}/{self.header_color}{key}|n [category={category}]{typ}:\n\n{value}"
