...
def _create_storage_boxes(storage, count=8):
    """Create a number of storage boxes for a character."""
    for idx in range(1, count + 1):
        StorageBox.objects.create(storage=storage, name=f"Box {idx}")


def _ensure_storage(char):
    """Ensure the character has the required storage boxes."""
    storage = char.storage
    if not storage.boxes.exists():
        _create_storage_boxes(storage)
    return storage


def _instantiate_pokemon(species_key: str, level: int, char):
    """Generate the base Pokémon instance."""
    try:
...
def _create_owned_pokemon(char, instance, ability, gender):
    """Create the OwnedPokemon database entry."""
...
def _finalize_pokemon(char, pokemon, level):
    """Set level, heal and store the Pokémon."""
...
def _create_starter(
    ...
    instance = _instantiate_pokemon(species_key, level, char)
    if not instance:
        return None

    pokemon = _create_owned_pokemon(char, instance, ability, gender)
    _finalize_pokemon(char, pokemon, level)
    return pokemon
...
class CmdChargen(Command):
    ...
    def func(self):
        if self.caller.db.validated:
            self.caller.msg(
                "You are already validated and cannot run chargen again."
            )
            return
        EnhancedEvMenu(
            self.caller,
    ...
    options = tuple(
        {
            "key": ab.lower(),
            "desc": ab,
            "goto": ("fusion_confirm", {"ability": ab}),
        }
        for ab in ab_list
    )
    +(
...
def starter_ability(caller, raw_string, **kwargs):
    ...
    if entry in ("starterlist", "starters", "pokemonlist"):
        ...
        return starter_species(
            caller, "", type=caller.ndb.chargen.get("favored_type")
        )

    key = STARTER_LOOKUP.get(entry)
    if not key:
        caller.msg(
            "Invalid starter species.\nUse 'starterlist' or 'pokemonlist'."
        )
        return starter_species(
            caller, "", type=caller.ndb.chargen.get("favored_type")
        )
        ...
        opts.append(
            {
                ...
                "exec": lambda cb, k=k: cb.ndb.chargen.__setitem__(
                    "ability", abilities[k]
                ),
                "goto": "starter_gender",
            }
        ...
        opts.append(
            {
                ...
                "exec": lambda cb: cb.ndb.chargen.__setitem__(
                    "ability", abilities["H"]
                ),
                "goto": "starter_gender",
            }
    ...
    opts.append(
        {
            "key": "_default",
            "exec": lambda cb: cb.msg(
                "Invalid choice. Please pick 1, 2… or H."
            ),
            "goto": "_repeat",
        }
    )
...
def finish_human(caller, raw_string):
    ...
    caller.msg(
        f"You received {pk.name} with ability {pk.ability} as your starter!"
    )
