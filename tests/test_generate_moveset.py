...
class DummyPokemon:
	def __init__(self):
		self.types = ["Ground"]
		self.stats = types.SimpleNamespace(attack=120, special_attack=60, speed=95, hp=90, special_defense=70)
		self.move_pool = [
			DummyMove("Earthquake", "Ground", "Physical", "tm_all"),
			DummyMove("Rock Slide", "Rock", "Physical", "level_up", ["coverage"]),
			DummyMove("Swords Dance", "Normal", "Status", "level_up", ["setup", "status"]),
			DummyMove("Stealth Rock", "Rock", "Status", "tm_all", ["hazard", "status"]),
