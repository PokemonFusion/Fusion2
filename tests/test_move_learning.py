...
def test_on_exit_called_when_auto_learn():
	...
	spec = importlib.util.spec_from_file_location("pokemon.utils.move_learning", path)
	...
	poke = DummyPokemon()
	called = []
	ml.learn_move(poke, "tackle", caller=None, prompt=True, on_exit=lambda *a: called.append(True))
	...
	if orig_enh is not None:
		sys.modules["utils.enhanced_evmenu"] = orig_enh
	else:
