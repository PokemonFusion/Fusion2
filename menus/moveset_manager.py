...
def node_manage(caller, raw_input=None):
	...
	if raw_input is None:
		...
		lines.append(
			("Enter a number to edit that set, or type 'swap <n>' to make it active. Type 'back' or 'b' to exit.")
		)
...
def node_edit(caller, raw_input=None):
	...
	if raw_input is None:
		current = (
			", ".join([s.move.name for s in sets.get(idx).slots.order_by("slot")] if idx in sets else []) or "(empty)"
		)
