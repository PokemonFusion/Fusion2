...
def _route_move(
	...
	**kwargs,
	...
	if not s:
		raise EvMenuGotoAbortMessage("Type a letter (A–D) or a move name. Type 'quit' to cancel.")
...
def _route_target(
	...
	**kwargs,
	...
	if not re.fullmatch(r"[AB]\d+", s):
		raise EvMenuGotoAbortMessage("Position must be like A1/B1 (names change when switching).")
