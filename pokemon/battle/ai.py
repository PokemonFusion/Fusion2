...
class AIMoveSelector:
	...
	def select_move(self, ai_level: int, ai_personality: str, pokemon, opponent, battle_state) -> Move:
		...
		for move in moves:
			...
			util = self._score_move_by_utility(move, pokemon, opponent, battle_state, ai_level)
	...
	def _score_move_by_utility(self, move: Move, pokemon, opponent, battle_state, ai_level: int) -> float:
		if ai_level < 3:
			return 0.0
	...
	def _apply_personality_weighting(self, damage_score: float, utility_score: float, personality: str) -> float:
...
def generate_moveset(pokemon, ai_level: int, ai_personality: str) -> List[Move]:
	...
	sources = AIMoveSelector.allowed_sources.get(ai_level, AIMoveSelector.allowed_sources[5])
	...
	def score(move: Move) -> float:
		...
		if ("heal" in tags or "utility" in tags) and ai_level >= 3:
			utility += 2
		if "status" in tags and (ai_personality == "tactician" or role == "utility_support"):
			utility += 2
		weights = AIMoveSelector.personality_weights.get(ai_personality, (1.0, 1.0))
	...
	if not any(
		getattr(m, "type", "").lower() in types and getattr(m, "category", "").lower() != "status" for m in selected
	):
		for _, mv in scored:
			if mv in selected:
				continue
			if getattr(mv, "type", "").lower() in types and (getattr(mv, "category", "").lower() != "status"):
				if selected:
					selected[-1] = mv
				else:
					selected.append(mv)
