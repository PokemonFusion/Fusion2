items = 
	...
	"aguavberry": {
		...
		onTryEatItem(item, pokemon) {
			if (!this.runEvent("TryHeal", pokemon, None, this.effect, pokemon.baseMaxhp / 3)) return False;
		},
		onEat(pokemon) {
	...
	"airballoon": {
		...
		onDamagingHit(damage, target, source, move) {
			...
			this.runEvent("AfterUseItem", target, None, None, this.dex.items.get("airballoon"));
		},
	...
	"berryjuice": {
		...
		onUpdate(pokemon) {
			if (pokemon.hp <= pokemon.maxhp / 2) {
				if (this.runEvent("TryHeal", pokemon, None, this.effect, 20) && pokemon.useItem()) {
					this.heal(20);
				}
			}
		},
	...
	"enigmaberry": {
		...
		onTryEatItem(item, pokemon) {
			if (!this.runEvent("TryHeal", pokemon, None, this.effect, pokemon.baseMaxhp / 4)) return False;
		},
		onEat() { },
		"num": 208,
		"gen": 3,
	...
	"oranberry": {
		...
		onTryEatItem(item, pokemon) {
			if (!this.runEvent("TryHeal", pokemon, None, this.effect, 10)) return False;
		},
		onEat(pokemon) {
			this.heal(10);
		},
	...
	"goldberry": {
		...
		onTryEatItem(item, pokemon) {
			if (!this.runEvent("TryHeal", pokemon, None, this.effect, 30)) return False;
		},
		onEat(pokemon) {
			this.heal(30);
		},
